...
function echopcpu($timerange)
{
    ...
    for ($cpuid = 0; $cpuid < $nproc; ++$cpuid) {
        ...
        $pcpu[$cpuid]['cpuid'] = $cpuid;
        $pcpu[$cpuid]['usage'] = percentage(($diffstat[$cpuid]['total'] - $diffstat[$cpuid]['idle']), $diffstat[$cpuid]['total'], 2);
    }
    //print_r($pcpu);
    $rst = array('cpustat' => $pcpu);
    $json = json_encode($rst);
    echo $json;
    exit();
}
